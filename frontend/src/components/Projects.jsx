...
export default function Projects() {
    ...
    useEffect(() => {
        ...
        if (showAll) {
            // Scroll to the 4th project (first hidden one)
            const target = document.getElementById("projects-more");
            scrollToWithOffset(target, 120); // offset = header height
        } else {
        ...
        <>
            <section id="projects" className="py-24 bg-slate-950 relative overflow-hidden">
                ...
                <div className="container mx-auto px6 relative z-10">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 m-5 gap-16">
                        {visibleProjects.map((project, index) => {
                            return (
                                
                                <div key={index} id={index === 3 ? "projects-more" : undefined}
                                    ...
                                    <div className='p-6'>
                                        ...
                                        {/* Description */}
                                        <p className='text-gray-300 text-sm mb-4 leading-relaxed max-h-16 overflow-hidden group-hover:max-h-96 transition-all duration-500'>
                                            {project.description}
                                        </p>

                                        <div className='flex flex-wrap gap-1 mb-4'>
                                            {project.technologies.map((tech, index) => (
                                                <span key={index} className='bg-green-600 text-white px-2 py-1 rounded-full text-xs font-medium'>
                                                    {tech}
                                                </span>
                                            ))}
                                        </div>
                    ...
                    {projects.length > 3 && (
                        <div
                            className={`text-center transition-all duration-1000 delay-1000 mt-5 ${isVisible
